...
public class ParticleManagerServer{
	...
	public Object spawn(World world, String name, double x, double y, double z, double targetX, double targetY, double targetZ){
		if (!world.isRemote){
			...
			writer.add(targetY);
			writer.add(targetZ);
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(world.provider.getDimension(), x, y, z, 32, AMPacketIDs.PARTICLE_SPAWN_SPECIAL, writer.generate());
		}
		return null;
	}
	...
	public Object spawn(World world, String name, Entity source, Entity target){
		if (!world.isRemote){
			...
			writer.add(target.getEntityId());
			AMNetHandler.INSTANCE.sendPacketToAllClientsNear(world.provider.getDimension(), source.posX, source.posY, source.posZ, 32, AMPacketIDs.PARTICLE_SPAWN_SPECIAL, writer.generate());
		}
		return null;
	}
