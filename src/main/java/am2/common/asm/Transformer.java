...
public class Transformer implements IClassTransformer {
		...
		for (MethodNode mn : cn.methods) {
			if (mn.name.equals("getLook") || (mn.name.equals("func_70676_i") && mn.desc.equals("(F)Lnet/minecraft/util/math/Vec3d"))) {
				...
				while (iter.hasNext()) {
					InsnList toAdd = new InsnList();
					toAdd.add(new VarInsnNode(ALOAD, 0));
					toAdd.add(new MethodInsnNode(INVOKESTATIC, "am2/common/utils/EntityUtils", "correctLook", "(Lnet/minecraft/util/math/Vec3d;Lnet/minecraft/entity/Entity;)Lnet/minecraft/util/math/Vec3d;", false));
					AbstractInsnNode ain = (AbstractInsnNode) iter.next();
			...
			if (mn.name.equals("getEyeHeight") || (mn.name.equals("func_70047_e") && mn.desc.equals("()F"))) {
				...
				while (iter.hasNext()) {
					...
					if (ain != null && ain.getOpcode() == FRETURN) {
						InsnList toAdd = new InsnList();
						toAdd.add(new VarInsnNode(ALOAD, 0));
						toAdd.add(new MethodInsnNode(INVOKESTATIC, "am2/common/utils/EntityUtils", "correctEyePos", "(FLnet/minecraft/entity/Entity;)F", false));
						LogHelper.info("Core: Located target ARETURN insn node");
						mn.instructions.insertBefore(ain, toAdd);
					}
				}
	...
	private byte[] alterEntityLivingBase(byte[] bytes, boolean is_obfuscated) {
		...
		
		{
			...
			method.instructions.add(new MethodInsnNode(INVOKESTATIC, "am2/common/utils/EntityUtils", "correctMouvement", "(FFFLnet/minecraft/entity/Entity;)Z", false));
