...
public class EntityFireGuardian extends AM2Boss{
	...
	private void flamethrower(){
		...
		if (world.isRemote){
			...
			List<EntityLivingBase> entities = world.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(2.5, 2.5, 2.5).addCoord(look.x * 3, 0, look.z * 3));
			for (EntityLivingBase ent : entities){
				if (ent == this)
					continue;
				ent.attackEntityFrom(DamageSources.causeFireDamage(this), 5);
	...
	protected void dropFewItems(boolean par1, int par2){
		...
		for (int j = 0; j < i; j++){
			this.entityDropItem(new ItemStack(ItemDefs.essence, 1, GameRegistry.findRegistry(Affinity.class).getKey(Affinity.FIRE)), 0.0f);
		}
