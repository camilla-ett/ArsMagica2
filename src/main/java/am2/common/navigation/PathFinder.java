	...
	private static boolean BlockIsNavigateable(World world, Point3D point, Point3D Current, Entity entity){
		...
		if (!isDiagonalMovement(point, Current)){
			...
			for (int i = 0; i < height; ++i){
				Block block = world.getBlockState(new BlockPos (unshifted.x, unshifted.y + i, unshifted.z)).getBlock();
				blocks[i] = block;
			}
			for (Block b : blocks){
			...
			for (int i = 0; i < height; i += 4){
				...
				Block block = world.getBlockState(new BlockPos (unshifted.x, unshifted.y + i, unshifted.z)).getBlock();
				blocks[i] = block;
				block = world.getBlockState(new BlockPos (unshifted.x + deltaX, unshifted.y + i, unshifted.z)).getBlock();
				blocks[i + 1] = block;
				block = world.getBlockState(new BlockPos (unshifted.x, unshifted.y + i, unshifted.z + deltaZ)).getBlock();
				blocks[i + 2] = block;
				block = world.getBlockState(new BlockPos (unshifted.x, unshifted.y + i + deltaY, unshifted.z)).getBlock();
				blocks[i + 3] = block;
			}
			for (Block b : blocks){
				boolean currentBlockValid = false;
