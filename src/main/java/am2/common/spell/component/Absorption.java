...
@Override
public boolean applyEffectBlock(ItemStack stack, World world, BlockPos blockPos, EnumFacing blockFace, double impactX,
...
public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target) {
	if (target instanceof EntityLivingBase) {
		...
		if (!world.isRemote)
			((EntityLivingBase)target).addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("minecraft:absorption"), duration, SpellUtils.countModifiers(SpellModifiers.BUFF_POWER, stack)));
