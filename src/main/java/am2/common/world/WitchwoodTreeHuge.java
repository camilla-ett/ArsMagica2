...
public class WitchwoodTreeHuge extends WorldGenAbstractTree{
	...
	void genTreeLayer(int par1, int par2, int par3, float par4, int par5, IBlockState par6){
	...
	void generateLeafNode(int par1, int par2, int par3){
		...
		for (int i1 = par2 + this.leafDistanceLimit; l < i1; ++l){
			...
			this.genTreeLayer(par1, l, par3, f, 1, BlockDefs.witchwoodLeaves.getDefaultState());
		}
	}
	...
	void generateTrunk(){
		...
		this.placeBlockLine(aint, aint1, BlockDefs.witchwoodLog.getDefaultState());
	...
	void generateLeafNodeBases(){
		...
		for (int[] aint = new int[]{this.basePos[0], this.basePos[1], this.basePos[2]}; i < j; ++i){
			...
			if (this.leafNodeNeedsBase(k)){
				this.placeBlockLine(aint, aint2, BlockDefs.witchwoodLog.getDefaultState());
			}
		}
	}
