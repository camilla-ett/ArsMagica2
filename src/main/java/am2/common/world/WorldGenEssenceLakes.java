...
public class WorldGenEssenceLakes extends WorldGenerator{
	...
	public boolean generate(World par1World, Random par2Random, BlockPos pos){
		...
		for (; pos.getY() > 5 && par1World.isAirBlock(pos); pos = pos.down());
			...
			for (i1 = 0; i1 < 16; ++i1){
				for (j2 = 0; j2 < 16; ++j2){
					for (i2 = 0; i2 < 8; ++i2){
						...
						if (flag){
							Material material = par1World.getBlockState(pos.add(i1, i2, j2)).getMaterial();
				...
				for (j2 = 0; j2 < 16; ++j2){
					for (i2 = 0; i2 < 8; ++i2){
						if (aboolean[(i1 * 16 + j2) * 8 + i2]){
							par1World.setBlockState(pos.add(i1, i2, j2),
									i2 >= 4 ? Blocks.AIR.getDefaultState() : this.targetBlock.getDefaultState(),
									2);
						}
					}
				}
